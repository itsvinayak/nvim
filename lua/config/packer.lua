local ensure_packer = function()
    ...
    if fn.empty(fn.glob(install_path)) > 0 then
        local output = fn.system({ "git", "clone", "--depth", "1", "https://github.com/wbthomason/packer.nvim",
            install_path })
        if vim.v.shell_error == 0 then
            vim.cmd [[runtime plugin/packer.vim]]
            return true
        else
            print("Error installing packer.nvim: " .. output)
            return false
        end
    end
    return false
